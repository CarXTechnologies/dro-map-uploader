...
public class MapManagerConfig : SingletonScriptableObject<MapManagerConfig>
{
	...
	public static bool TryGetAttach(ulong id,
		out AttachData attachData)
	{
		attachData = GetAttach(id);
		return attachData != null;
	}

	public static bool GetOrAttach(ulong id,
		out AttachData attachData)
	{
	...
	public static AttachData Attach(ulong id,
		MapMetaConfig config)
	{
		...
		else
		{
			...
			instance.attachingConfigs.Add(new AttachData
			{
				id = id,
				metaConfig = config
			});
