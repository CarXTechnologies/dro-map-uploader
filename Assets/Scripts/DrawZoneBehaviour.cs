...
public class DrawZoneBehaviour : MonoBehaviour
{
	...
	private void RayMaxDraw(ref Vector3 pos,
		ref Vector3 scale,
		Vector3 direction)
	{
		if (Physics.Raycast(new Ray(pos, direction), out var recastHit))
		{
			Gizmos.color = Color.red;
			Gizmos.DrawRay(pos, recastHit.distance * direction);

			pos += (recastHit.distance * direction - new Vector3(scale.x * direction.x, scale.y * direction.y, scale.z * direction.z) / 2);
		}
	}
}