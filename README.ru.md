...
9. Перетащите объект из сцены в созданную папку `Prefabs` — теперь его можно использовать сколько угодно раз.
   ...
   <img src="Image/3.png" alt="Меню Prefab → Unpack Completely" style="width:300px;"/>
...
На скриншотах Road и SpawnPoint показано старое меню маркеров. В текущей версии используйте SpawnPoint, Road или Animation; Ambient не поддерживается.
...
В **Build → Optimization** доступны разбиение и объединение по секторам, отдельное упрощение коллайдеров и галочка **Generate sector LODs** для генерации LOD. Настройки и исключения описаны в [Build optimization](BuildOptimization.md).
...
Подробнее: [экспорт Wavefront](Wavefront.md). Параметры сборки Binary описаны в [настройках сборки](#настройки-сборки); отдельной спецификации бинарного формата в этом репозитории нет.
