...
| **Format** | Формат упаковки мода: **Wavefront** или **Binary** — см. [Формат мода](#формат-мода-wavefront-и-binary). |
| **Binary textures** | BC7 или RGBA32 с готовыми mip-уровнями. Сжатие контейнера применяется автоматически. |
...
Оба формата экспортируют все корни сцены напрямую и поддерживают одинаковые компоненты. После смены формата пересоберите **Map и Meta**. Поддерживаются только карты (`contentType: "map"`).

Подробнее: [экспорт Wavefront](Wavefront.md) и [бинарный формат](Assets/Plugins/CarX.Modding.Creator/BinaryFormat.md).
...
Volume и ReflectionProbe разрешены только для превью и не обязательны. UI, частицы, видео и Joint не экспортируются; валидатор сообщает об ошибке. Правила и лимиты находятся в `Assets/Editor/MapSceneRules.cs`.
